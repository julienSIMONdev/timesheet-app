      ...
      <div style={{ backgroundColor: t.card, border: `2px solid ${t.border}` }} className="rounded-lg p-6 w-full max-w-2xl my-8">
        ...
        <div className="space-y-4">
          ...
          <div style={{ borderTop: `1px solid ${t.border}` }} className="pt-4">
            ...
            <div className="mt-4"><label style={{ color: t.text }} className="block text-sm font-medium mb-2">Catégorie</label><select value={cat} onChange={e => { setCat(e.target.value); setSubCat(''); }} style={inp} className="w-full px-4 py-2 rounded-lg"><option value="">Sélectionner</option><option value="trajet">Trajet</option><option value="pose">Pose</option><option value="atelier">Atelier</option></select></div>
            ...
            {(cat === 'pose' || cat === 'atelier' || cat === 'trajet') && <><div className="mt-4"><label style={{ color: t.text }} className="block text-sm font-medium mb-2">Client</label><select value={client} onChange={e => { setClient(e.target.value); setProjectId(''); }} style={inp} className="w-full px-4 py-2 rounded-lg"><option value="">-- Aucun --</option>{clients.map(c => <option key={c} value={c}>{c}</option>)}</select></div>{client && cat !== 'trajet' && <div className="mt-4"><label style={{ color: t.text }} className="block text-sm font-medium mb-2">Projet</label><select value={projectId} onChange={e => setProjectId(e.target.value)} style={inp} className="w-full px-4 py-2 rounded-lg"><option value="">-- Aucun --</option>{clientProjects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>}</>}
            ...
            {(cat === 'pose' || cat === 'atelier') && client && <div className="flex items-center mt-4"><input type="checkbox" id="ex" checked={extra} onChange={e => setExtra(e.target.checked)} className="w-5 h-5 mr-2" style={{ accentColor: t.accent }} /><label htmlFor="ex" style={{ color: t.text }} className="text-sm">Travaux supplémentaires</label></div>}
...
function EmployeeView({ currentUser, timeEntries, projects, onDelete, onEdit, onAdd, t }) {
  ...
  const byDate = entries.reduce((a, e) => { if (!a[e.date]) a[e.date] = []; a[e.date].push(e); return a; }, {});
  const dates = Object.keys(byDate).sort().reverse();
  const total = entries.reduce((s, e) => s + e.hours, 0);
  const paniers = countPaniers(entries);

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h2 style={{ color: t.accent }} className="text-2xl font-bold">Mes heures</h2><button onClick={onAdd} style={{ backgroundColor: t.btn, color: t.btnText }} className="flex items-center px-4 py-2 rounded-lg font-semibold"><Plus className="w-5 h-5 mr-2" />Ajouter</button></div>
      <div className="grid grid-cols-2 gap-4">
        <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}` }} className="p-6 rounded-lg"><p style={{ color: t.textMuted }}>Total heures</p><p style={{ color: t.accent }} className="text-3xl font-bold">{formatHours(total)}</p></div>
        <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}` }} className="p-6 rounded-lg"><p style={{ color: t.textMuted }}>Paniers</p><p style={{ color: t.accent }} className="text-3xl font-bold">{paniers}</p></div>
      </div>
      <div className="space-y-4">
        {dates.map(d => {
                  ...
                  return (
                    <div key={e.id} style={{ backgroundColor: t.bg, border: `1px solid ${t.border}` }} className="flex justify-between items-start p-3 rounded">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2"><span style={{ color: t.text }} className="font-semibold capitalize">{e.category}{e.subCategory ? ` - ${e.subCategory}` : ''}</span>{e.category !== 'panier' && <span style={{ color: t.textMuted }}>{e.startTime} - {e.endTime} ({formatHours(e.hours)})</span>}{e.isExtra && <span style={{ backgroundColor: t.btn, color: t.btnText }} className="text-xs px-2 py-1 rounded">Trav. sup</span>}</div>
...
function PatronView({ timeEntries, projects, onAddProject, onDeleteProject, onDeleteClient, onDeleteEntry, onDeleteUser, users, currentUser, t }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}` }} className="p-6 rounded-lg"><p style={{ color: t.textMuted }}>Prévues</p><p style={{ color: t.accent }} className="text-3xl font-bold">{formatHours(totEst)}</p></div>
        <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}` }} className="p-6 rounded-lg"><p style={{ color: t.textMuted }}>Facturables</p><p style={{ color: '#22c55e' }} className="text-3xl font-bold">{formatHours(totBill + totTrajet)}</p></div>
        <div style={{ backgroundColor: t.card, border: `1px solid ${t.border}` }} className="p-6 rounded-lg"><p style={{ color: t.textMuted }}>Trav. sup</p><p style={{ color: '#f97316' }} className="text-3xl font-bold">{formatHours(totExtra)}</p></div>
      </div>
      <div className="space-y-4">
        {stats.map(p => (
          <div key={p.id} style={{ backgroundColor: t.card, border: `1px solid ${t.border}` }} className="rounded-lg p-6">
            ...
            <div><div className="flex justify-between mb-1"><span style={{ color: t.accent }} className="text-xs font-semibold">{p.pct.toFixed(0)}%</span><span style={{ color: p.remain >= 0 ? '#22c55e' : '#ef4444' }} className="text-xs font-semibold">{p.remain >= 0 ? 'Reste' : 'Dépassement'}: {formatHours(Math.abs(p.remain))}</span></div><div style={{ backgroundColor: t.bg }} className="h-2 rounded overflow-hidden"><div style={{ width: `${Math.min(p.pct, 100)}%`, backgroundColor: p.pct > 100 ? '#ef4444' : p.pct > 80 ? '#eab308' : '#22c55e' }} className="h-full" /></div></div>
          </div>
        ))}
      </div>
